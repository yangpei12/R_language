...
# 将cutreeStatic返回的数值转换为布尔值，进行布尔索引以保留未被切割的样本。
keepSamples <- (clust == 1)
...
nGenes <- ncol(datExpr)
block_thred <- 40000
# 在转录组数据中由于基因众多，因此WGCNA允许将表达谱数据分成不同的块（block），逐块执行网络构建
# 最后，特征基因高度相关的模块被合并，我们设定的每个块中最大的基因书量是 1.1 * nGenes
...
## 绘制聚类图和共表达模块
# 由于我们设定的maxBlockSize中包含的基因数量较多，超过了3600各，
# 因此本次分析的表达谱只有一个块。所以net$blocks都为1
...
## 选择模块
module <- "turquoise"   # 在模块和表型相关性分析中，和体重最相关的模块是brown
